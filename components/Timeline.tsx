...
export const Timeline: React.FC<TimelineProps> = ({ data }) => {
  ...
  return (
    <div ref={containerRef} className="relative pl-8 md:pl-0 max-w-4xl mx-auto">
      <div className="absolute left-0 md:left-1/2 top-0 bottom-0 w-[2px] bg-slate-800/50 -translate-x-1/2">
        <motion.div style={{ height: lineHeight }} className="w-full bg-gradient-to-b from-emerald-500 to-cyan-500 origin-top shadow-[0_0_15px_rgba(16,185,129,0.3)]" />
      </div>
      ...
      <div className="space-y-24">
        {data.map((item, index) => {
          ...
          // Logic to handle both single image string and multiple image array
          ...
          return (
            ...
            >
              <div className="absolute left-0 md:left-1/2 -translate-x-1/2 w-5 h-5 bg-slate-950 border-2 border-emerald-400 rounded-full z-10">
                <motion.div animate={{ scale: [1, 1.5, 1], opacity: [0.5, 0, 0.5] }} transition={{ repeat: Infinity, duration: 2 }} className="absolute inset-0 rounded-full bg-emerald-400" />
              </div>
                ...
                <div className="group bg-slate-900/40 border border-slate-800/50 rounded-3xl hover:border-emerald-500/50 transition-all duration-500 backdrop-blur-xl overflow-hidden hover:shadow-[0_20px_50px_rgba(0,0,0,0.3)]">
                  ...
                  {imageList.length > 0 && (
                    <div className={`grid gap-1 border-b border-slate-800/50 ${
                      ...
                      {imageList.map((img, i) => (
                        <div key={i} className={`overflow-hidden h-48 ${
                  ...
                  <div className="p-8">
                    <span className="inline-block px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-400 text-xs font-bold mb-4 tracking-widest">
                      {item.year}
                    </span>
                    <h3 className="text-2xl font-display font-bold text-white mb-1 group-hover:text-emerald-400 transition-colors">{item.role}</h3>
                    <h4 className="text-slate-400 font-medium mb-4 uppercase tracking-wider text-xs">{item.company}</h4>
                    <p className="text-slate-500 text-sm leading-relaxed group-hover:text-slate-300 transition-colors">{item.description}</p>
                  </div>
                </div>
              </div>
            </motion.div>
